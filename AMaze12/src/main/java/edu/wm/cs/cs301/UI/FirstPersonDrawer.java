 ...
 */
public class FirstPersonDrawer implements Serializable{
    ...
    public FirstPersonDrawer(int width, int height, int map_unit, int step_size, Cells mazecells, Cells seencells, int map_scale, int[][] mazedists, int mazew, int mazeh, BSPNode bsp_root){
    	...
    	Log.v("FPDrawer", "in FPD constructor");
        ...
        Log.v("fpd",""+ mazecells.width);
        Log.v("fpd",""+mazecells.height);
        Log.v("fpd",""+seencells.width);
        Log.v("fpd",""+seencells.height);
 ...
 //      PlayMaze.gw.setColor("BLACK");
    //   Log.v("fpd", "set first color");
       // Log.v("fpd", PlayMaze.gw.count + " " + PlayMaze.gw.color);
      //  Log.v("fpd", "" + view_width);
        PlayMaze.gw.fillRect(0, 0, view_width, view_height/2, "BLACK");
//        // draw dark gray background on upper half
        PlayMaze.gw.setColor("GRAY");
        PlayMaze.gw.fillRect(0, view_height/2, view_width, view_height/2, "GRAY");
//        // set color to white and draw what ever can be seen from the current position
 //       PlayMaze.gw.setColor("WHITE");
    ...
    private boolean bbox_visible(int ymax, int ymin, int xmin, int xmax) {
        ...
        coordinates[0]=x1;
        coordinates[1] = x2;
        return (rset.intersect(coordinates));
    }
    /**
    ...
    /**
     * Draws segment on screen via graphics attribute gw
    ...
    private void drawrect(Seg seg, int ox1, int y1, int ox2, int y2) {
        ...
        int x1i = x1;
        int xd = x2-x1;
       // Log.v("fpd", "creating a color");
       // Log.v("fpd", "colors: " + seg.colorArray[0] + " " + seg.colorArray[1] + " " + seg.colorArray[2]);
        //int color = PlayMaze.gw.color.rgb(seg.colorArray[0], seg.colorArray[1], seg.colorArray[2]);
        //PlayMaze.gw.setColor(color);
       // PlayMaze.gw.p.setColor(Color.RED);
        //gw.setColor(seg
        //System.out.println("first person drawer color "+ seg.gw.col.toString());
        boolean drawn = false;
        ...
        while (x1i <= x2) {
            ...
            coordinates[0] = x1i;
            coordinates[1] = x2;
            if (!rset.intersect(coordinates))
                break;
            x1i = coordinates[0];
            int x2i = coordinates[1];
            ...
            int color = PlayMaze.gw.createColor(seg.colorArray[0], seg.colorArray[1], seg.colorArray[2]);
            wallpaint.setColor(color);
            Path wallpath = new Path();
            wallpath.reset();
            wallpath.moveTo(xps[0],yps[0]);
            wallpath.lineTo(xps[1],yps[1]);
            wallpath.lineTo(xps[2],yps[2]);
            wallpath.lineTo(xps[3],yps[3]);
            wallpath.lineTo(xps[0], yps[0]);
            PlayMaze.gw.canvas.drawPath(wallpath, wallpaint);
    ...
    private void udpateSeenCellsForSegment(Seg seg) {
        ...
        int sdsx = getSign(sdx);
        int sdsy = getSign(sdy);
